...
# Find the latest tag of the form "imms-X.Y" and generate a version from it.
LATEST_TAG:=$(shell git tag | grep -Exe 'imms-[0-9]+(\.[0-9]+)?' | sort -rn | head -1)
COMMITS_SINCE:=$(shell git rev-list $(LATEST_TAG)..HEAD --count)
